...
instead of becoming a tag nothing will query again.
  ...
  journ tags --json`,
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			counts, err := collectTags(journ, project)
			if err != nil {
				return err
			}
...
func collectTags(journ *client.Client, project string) ([]tagCount, error) {
	...
	todos, err := journ.ListTodos(project, client.TodoFilter{})
	if err != nil {
		return nil, err
	}
	...
	plans, err := journ.ListPlans(project, client.PlanFilter{})
	if err != nil {
		return nil, err
	}
	...
	logs, err := journ.ListLogs(project, client.LogFilter{})
	if err != nil {
		return nil, err
	}
	...
	docs, err := journ.ListDocs(project, client.DocFilter{})
	if err != nil {
		return nil, err
	}
...
func renderTags(counts []tagCount) error {
	...
	for _, entry := range counts {
		if !entry.Known {
			fmt.Fprintln(os.Stderr, "\n* not in the known vocabulary; `journ tags --unused` lists what is")
			break
		}
	}
	return nil
