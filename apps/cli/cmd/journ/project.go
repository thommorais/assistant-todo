...
func projectCreateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			project, err := journ.CreateProject(client.CreateProjectInput{
				Name:  args[0],
				Slug:  slug,
				Descr: descr,
			})
			...
			fmt.Fprintf(os.Stderr, "\nselect it with: eval \"$(journ use %s)\"\n", project.Slug)
			return nil
		},
	}
...
func projectUpdateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			project, err := journ.UpdateProject(ref, in)
			if err != nil {
				return err
			}
			if flagJSON {
...
func projectDeleteCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			if err := journ.DeleteProject(args[0]); err != nil {
				return err
			}
			if !flagJSON {
...
func projectMemberCommand() *cobra.Command {
	...
	add := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			project, err := journ.AddMember(ref, args[0], role)
			if err != nil {
				return err
			}
			if flagJSON {
	...
	promote := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			project, err := journ.SetMemberRole(ref, args[0], newRole)
			if err != nil {
				return err
			}
			if flagJSON {
	...
	remove := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			project, err := journ.RemoveMember(ref, args[0])
			if err != nil {
				return err
			}
			if flagJSON {
...
func projectListCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			projects, err := journ.ListProjects(archived)
			if err != nil {
				return err
			}
...
func projectGetCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			project, err := journ.GetProject(ref)
			if err != nil {
				return err
			}
			if flagJSON {
