...
func TestListProjects(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`{"projects":[{"id":"pr1","slug":"journ","name":"journ","descr":"A journal","archived":false,"members":[{"user_id":"u1","email":"a@b.c","role":"owner"}],"created_at":"2026-09-11T01:07:01Z","updated_at":"2026-09-11T01:07:01Z"}]}`))
	}))
	defer server.Close()
...
func TestGetProjectAcceptsAnIDOrSlug(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath = r.URL.Path
		_, _ = w.Write([]byte(`{"id":"pr1","slug":"journ","name":"journ","archived":false,"members":[],"created_at":"2026-09-11T01:07:01Z","updated_at":"2026-09-11T01:07:01Z"}`))
	}))
	defer server.Close()

	project, err := New(server.URL, "tok").GetProject("journ")
	if err != nil {
		t.Fatalf("GetProject() error = %v", err)
	}
	if gotPath != "/api/journ/projects/journ" {
		t.Errorf("path = %q", gotPath)
	}
	if project.Slug != "journ" {
...
func TestCreateProject(t *testing.T) {
	...
	if gotMethod != http.MethodPost || gotPath != "/api/journ/projects" {
...
func TestUpdateProject(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotMethod, gotPath = r.Method, r.URL.Path
		_ = json.NewDecoder(r.Body).Decode(&gotBody)
		_, _ = w.Write([]byte(`{"id":"pr1","slug":"journ","name":"Renamed","archived":true,"members":[],"created_at":"2026-09-11T01:07:01Z","updated_at":"2026-09-11T01:07:01Z"}`))
	}))
	defer server.Close()

	archived := true
	project, err := New(server.URL, "tok").UpdateProject("journ", ProjectInput{Archived: &archived})
	if err != nil {
		t.Fatalf("UpdateProject() error = %v", err)
	}
	if gotMethod != http.MethodPatch || gotPath != "/api/journ/projects/journ" {
		t.Errorf("%s %s", gotMethod, gotPath)
	}
...
func TestDeleteProject(t *testing.T) {
	...
	if err := New(server.URL, "tok").DeleteProject("journ"); err != nil {
		t.Fatalf("DeleteProject() error = %v", err)
	}
	if gotMethod != http.MethodDelete || gotPath != "/api/journ/projects/journ" {
		t.Errorf("%s %s", gotMethod, gotPath)
	}
}
...
func TestAddMember(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath = r.URL.Path
		_ = json.NewDecoder(r.Body).Decode(&gotBody)
		_, _ = w.Write([]byte(`{"id":"pr1","slug":"journ","name":"journ","archived":false,"members":[{"user_id":"u1","role":"owner"},{"user_id":"u2","email":"b@c.d","role":"editor"}],"created_at":"2026-09-11T01:07:01Z","updated_at":"2026-09-11T01:07:01Z"}`))
	}))
	defer server.Close()

	project, err := New(server.URL, "tok").AddMember("journ", "b@c.d", "editor")
	if err != nil {
		t.Fatalf("AddMember() error = %v", err)
	}
	if gotPath != "/api/journ/projects/journ/members" {
		t.Errorf("path = %q", gotPath)
	}
...
func TestRemoveMember(t *testing.T) {
	...
	if _, err := New(server.URL, "tok").RemoveMember("journ", "u2"); err != nil {
		t.Fatalf("RemoveMember() error = %v", err)
	}
	if gotMethod != http.MethodDelete || gotPath != "/api/journ/projects/journ/members/u2" {
		t.Errorf("%s %s", gotMethod, gotPath)
	}
}
...
func TestSetMemberRole(t *testing.T) {
	...
	if _, err := New(server.URL, "tok").SetMemberRole("journ", "u2", "owner"); err != nil {
		t.Fatalf("SetMemberRole() error = %v", err)
	}
	if gotMethod != http.MethodPatch || gotPath != "/api/journ/projects/journ/members/u2" {
		t.Errorf("%s %s", gotMethod, gotPath)
	}
