...
URL="${JOURN_URL:-http://127.0.0.1:8090}"
SEED_EMAIL="${JOURN_DEV_EMAIL:-user@test.com}"
SEED_PASSWORD="${JOURN_DEV_PASSWORD:-pass@test}"

if curl -fsS -m 2 "$URL/api/health" > /dev/null 2>&1; then
	echo "journ: a server is already listening on ${URL#http://}." >&2
	echo "       This script starts and stops its own; stop that one first," >&2
	echo "       or point JOURN_URL elsewhere." >&2
	exit 1
fi
...
SERVER_LOG="$(mktemp -t journ-dev-server)"
...
for _ in $(seq 1 100); do
	...
	if ! kill -0 "$SERVER_PID" 2> /dev/null; then
		echo "journ: the server exited before becoming healthy:" >&2
		cat "$SERVER_LOG" >&2
		rm -f "$SERVER_LOG"
		exit 1
	fi
...
if ! curl -fsS -m 1 "$URL/api/health" > /dev/null 2>&1; then
	echo "journ: the server did not become healthy in time:" >&2
	cat "$SERVER_LOG" >&2
	rm -f "$SERVER_LOG"
	exit 1
fi
...
if [[ -z ${JOURN_TOKEN:-} ]]; then
	...
	if [[ -z $JOURN_TOKEN ]]; then
		echo "journ: could not authenticate as $SEED_EMAIL." >&2
