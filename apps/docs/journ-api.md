...
```bash
curl -X POST localhost:8090/api/journ/projects/search-rewrite/tickets \
  -H "Authorization: $TOKEN" -d '{
    "title":"Search ranking ignores recency","priority":"high",
    "external_ref":"JOURN-31","tags":["search","bug"]}'   # -> slug "search-ranking-ignores-recency"

curl -X PATCH localhost:8090/api/journ/tickets/$ID -H "Authorization: $TOKEN" \
