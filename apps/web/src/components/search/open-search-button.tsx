import { Search } from 'lucide-react'
import { useSearchStore } from '_/app/search-store'

export const OpenSearchButton = () => {
	const setOpen = useSearchStore(state => state.setOpen)

	return (
		<button
			type='button'
			onClick={() => setOpen(true)}
			className='text-muted-foreground hover:text-foreground relative hidden w-full min-w-[250px] cursor-pointer items-center justify-start gap-2 text-sm font-normal transition-colors md:flex md:w-40 lg:w-64'
		>
			<Search size={18} />
			<span>Find anything...</span>
			<kbd className='bg-accent border-border pointer-events-none absolute top-0 right-0 hidden h-5 items-center gap-1 border px-1.5 text-[10px] font-medium opacity-0 transition-opacity select-none group-hover:opacity-100 sm:flex'>
				<span className='text-xs'>⌘</span>K
			</kbd>
		</button>
	)
}
