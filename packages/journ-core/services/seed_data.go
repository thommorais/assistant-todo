	...
	return []seedSpec{
		{
			...
			plans: []ports.CreatePlanInput{
				...
				{
					Title:  "MCP adapter",
					Goal:   "Expose the same use cases as MCP tools so Claude can read and write the journal directly.",
...
code compiles and unit-tests clean while still being unloadable.`,
				...
				{
					Title: "Reworked logs from event stream to work journal",
...
Dates stay server-assigned: a journal is only useful read chronologically if
