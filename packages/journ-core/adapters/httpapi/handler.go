...
// Handler wires the journ use cases onto PocketBase's router. It depends only
...
// rest of its own surface, so journ takes its own namespace.
const BasePath = "/api/journ"
...
func (h *Handler) Mount(e *core.ServeEvent) {
	...
	g.GET("/projects/{project}/logs", h.listLogs)
	g.POST("/projects/{project}/logs", h.writeLog)
	g.GET("/logs/{log}", h.getLog)
	g.PATCH("/logs/{log}", h.updateLog)
	g.POST("/logs/{log}/append", h.appendLog)
	g.DELETE("/logs/{log}", h.deleteLog)
